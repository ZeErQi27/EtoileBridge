...
class ArcpkgBundleMerger {
    ...
    private fun copyDirectory(
        ...
        val disabledChartPaths = if (override == null) {
            emptySet()
        } else {
            entries.firstOrNull { it.name.removePrefix(prefix) == settingsFile }
                ?.let { disabledChartPaths(zip, it, override) }
                .orEmpty()
        }
        entries.forEach { entry ->
            val relative = entry.name.removePrefix(prefix)
            if (relative.replace('\\', '/') in disabledChartPaths) {
                return@forEach
            }
    ...
    private fun disabledChartPaths(
        zip: ZipFile,
        settingsEntry: ZipEntry,
        override: BundleEntryOverride,
    ): Set<String> {
        if (override.chartOverrides.values.none { it.enabled == false }) {
            return emptySet()
        }
        return runCatching {
            zip.getInputStream(settingsEntry).use { input ->
                val project = EtoileYaml.decodeFromStream(ProjectInformation.serializer(), input)
                project.charts.mapIndexedNotNull { index, chart ->
                    val chartOverride = override.chartOverrides[chart.bundleChartKey(index)]
                    if (chartOverride?.enabled == false) {
                        chart.chartPath.replace('\\', '/').takeIf { it.isNotBlank() }
                    } else {
                        null
                    }
                }.toSet()
    ...
    private fun rewriteProjectInformation(input: InputStream, override: BundleEntryOverride): String {
        ...
        val rewritten = project.copy(
            charts = project.charts.mapIndexedNotNull { index, chart ->
                val chartOverride = override.chartOverrides[chart.bundleChartKey(index)]
                if (chartOverride?.enabled == false) {
                    null
                } else {
