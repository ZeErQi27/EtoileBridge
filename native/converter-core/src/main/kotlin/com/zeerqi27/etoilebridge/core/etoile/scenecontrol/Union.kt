...
object UnionSerializer : KSerializer<Union> {
    ...
    override fun deserialize(decoder: Decoder): Union {
        val jsonDecoder = decoder as? JsonDecoder ?: throw SerializationException("Union type can only be used with EtoileJson")
        ...
        return when (val element: JsonElement = jsonDecoder.decodeJsonElement()) {
            ...
            is JsonPrimitive -> {
                if (element.isString) {
                    ...
                    throw SerializationException("Unknown primitive type in EtoileJson")
                }
            }

            else -> throw SerializationException("Unsupported EtoileJson element type")
        }
    }
}
